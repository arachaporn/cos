...
<body>
	...
	<table style="border:1px solid #000;border-bottom:none;width:100%;font-size:0.9em;margin:5% 0 0 0;" cellpadding="0" cellspacing="0">
		<tr>
			...
			<td style="border-right:1px solid #000;border-bottom:1px solid #000;text-align:center;width:11.4%;">Date</td>
			<td style="border-right:1px solid #000;border-bottom:1px solid #000;text-align:center;width:15.5%;">Customer</td>
			<td style="border-right:1px solid #000;border-bottom:1px solid #000;text-align:center;width:12.5%;">PO. No.</td>
	...
	<?php 
	if($i==1){$pm=25;}elseif($i==2){$pm=30;}elseif($i==3){$pm=32;}elseif($i==4){$pm=31;}elseif($i==5){$pm=26;}elseif($i==5){$pm=27;}elseif($i==6){$pm=45;}elseif($i==7){$pm=0;}
	$total=0;
	$reject=0;
	$sum_reject=0;
	$sql="select * from sm_sales_balance where month_visited='".$month."'";
	$sql .=" and year_visited='".$year."' and project_manager='".$pm."'";
	...
	<?php for($j=0;$j<=2;$j++){?>
		...
		?>
		<tr>
			<td style="border-left:1px solid #000;border-right:1px solid #000;border-bottom:1px solid #000;width:8.5%;text-align:center;"><?php if($i==7){echo 'N/A';}elseif($pm != 27){echo substr($rs_acc2['username'],2);}else{echo $rs_acc2['username'];}?></td>
			...
			<td style="border-right:1px solid #000;border-bottom:1px solid #000;width:12.5%;"></td>
	...
	<table style="width:100%;font-size:0.9em;line-height:1.5em;" cellpadding="0" cellspacing="0">
		<tr>
			...
			<td style="border-right:1px solid #000;border-bottom:1px solid #000;vertical-align:top;width:11.5%;padding:0.5%;"><?php echo $rs['date_visited'].'/'.$rs['month_visited'].'/'.$rs['year_visited']?></td>
			...
			<td style="border-right:1px solid #000;border-bottom:1px solid #000;vertical-align:top;width:12.5%;padding:0.5%;"><?php echo $rs['po_no']?></td>
			...
			<td style="border-right:1px solid #000;border-bottom:1px solid #000;vertical-align:top;width:21%;padding:0.5%;"><?php echo $style_red;echo $rs['note'];echo $style_end_red;?></td>	
		</tr>		
	</table>
	<?php }//end while?>
	...
	<?php }//end if i?>	
	<?php }//end for pm1-pm6?>
	<!--<table style="border:1px solid #000;border-bottom:none;border-top:none;width:100%;font-size:0.9em;line-height:1.8em;" cellpadding="0" cellspacing="0">
		<?php
		$total=0;
		$sql="select * from sm_sales_balance where month_visited='".$month."'";
		...
		?>
		<tr>
			<td style="border-right:1px solid #000;border-bottom:1px solid #000;text-align:center;vertical-align:top;width:7.7%;padding:0.5%;">
				<?php
				$sql_acc="select * from account where id_account='".$rs['project_manager']."'";
				$res_acc=mysql_query($sql_acc) or die ('Error '.$sql_acc);
				$rs_acc=mysql_fetch_array($res_acc);
			...
			<td style="border-right:1px solid #000;border-bottom:1px solid #000;vertical-align:top;width:11%;padding:0.5%;"><?php echo $rs['date_visited'].'/'.$rs['month_visited'].'/'.$rs['year_visited']?></td>
			<td style="border-right:1px solid #000;border-bottom:1px solid #000;vertical-align:top;width:15%;padding:0.5%;">
				<?php
				$sql_customer="select * from company where id_company='".$rs['id_customer']."'";
				$res_customer=mysql_query($sql_customer) or die ('Error '.$sql_customer);
				$rs_customer=mysql_fetch_array($res_customer);
			...
			<td style="border-right:1px solid #000;border-bottom:1px solid #000;vertical-align:top;width:12%;padding:0.5%;"><?php echo $rs['po_no']?></td>
			<td style="border-right:1px solid #000;border-bottom:1px solid #000;vertical-align:top;width:20%;padding:0.5%;">
				<?php
				$sql_product="select * from product where id_product='".$rs['id_product']."'";
				$res_product=mysql_query($sql_product) or die ('Error '.$sql_product);
				$rs_product=mysql_fetch_array($res_product);
			...
			<td style="border-right:1px solid #000;border-bottom:1px solid #000;text-align:right;vertical-align:top;width:10%;padding:0.5%;"><?php echo number_format($rs['quantities'],2)?></td>
			<td style="border-right:1px solid #000;border-bottom:1px solid #000;text-align:right;vertical-align:top;width:15%;padding:0.5%;"><?php echo number_format($rs['price_per_baht'],2)?></td>
			<td style="border-right:1px solid #000;border-bottom:1px solid #000;text-align:right;vertical-align:top;width:15%;padding:0.5%;"><?php echo number_format($rs['total'],2)?></td>			
			<td style="border-bottom:1px solid #000;vertical-align:top;width:20.9%;padding:0.5%;"><?php echo $rs['note']?></td>	
		</tr>
		<?php }?>
	</table>-->
	<?php
	...
	$sum_reject2=0;
	$grand_total=0;
	$sql2="select * from sm_sales_balance where month_visited='".$month."'";
	...
	<table style="width:100%;font-size:0.8em;line-height:1.8em;margin:0 0 0 20%;" cellpadding="0" cellspacing="0">	
		<?php 
		$sql_target="select * from sm_sales_target where month_visited='".$month."'";
		$sql_target .=" and year_visited='".$year."'";
...
<?php
	...
	$mpdf-> SetHTMLHeader('<table cellspacing="0" cellpadding="0" style="border:1px solid #000;width:100%;"><tr>
			...
			<td colspan="7" style="vertical-align:bottom;padding:3% 2%;font-family:Arial;"><span style="font-size:1.5em;"><b>Sales Balance on '.$month_name.'&nbsp;'.$year.'</b><br></td>
			</tr></table>
