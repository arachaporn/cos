...
<body>
	...
	<table style="border: none; width: 100%; line-height:2.0em;" cellpadding="0" cellspacing="0" id="tb-quotation">
		<tr>
			<td colspan="7" style="font-size:2.5em; padding:3.5% 0 2% 0; text-align:center;"><b>ใบเสนอราคา</b></td>
		</tr>
		<tr>
			<td rowspan="2" style="font-size:1.5em; text-align:left; padding:1.5%; border-left:1px solid #000; border-top:1px solid #000;border-right:1px solid #000;">ชื่อบริษัท</td>
			<td rowspan="2" colspan="2" style="font-size:1.5em; text-align:left; padding:1.5%; border-top:1px solid #000;border-right:1px solid #000;">
				<?php
			...
			<td colspan="2" style="font-size:1.5em; text-align:left; padding:1.5%; border-top:1px solid #000; border-right:1px solid #000;">เลขที่ใบเสนอราคา </td>
			<td style="font-size:1.5em; text-align:left; padding:1.5%; border-top:1px solid #000; border-right:1px solid #000;"><?php echo $rs_quo['quotation_no']?></td>
		</tr>
		<tr>
			<td colspan="2" style="font-size:1.5em; text-align:left; padding:1.5%; border-right:1px solid #000;">วันที่</td>
			<td colspan="3" style="font-size:1.5em; text-align:left; padding:1.5%; border-right:1px solid #000;"><?php echo date("d/m/Y")?></td>
		</tr>
		<tr>
		<?php
		...
		?>
			<td style="font-size:1.5em; text-align:left; padding:1.5%; border-left:1px solid #000;border-right:1px solid #000;">ชื่อผู้ติดต่อ/เบอร์ </td>
			<td colspan="2" style="font-size:1.5em; text-align:left; padding:1.5%;border-right:1px solid #000;"><?php echo $rs_contact['contact_name']?></td>
			<td colspan="2" style="font-size:1.5em; text-align:left; padding:1.5%;border-right:1px solid #000;">ผู้เสนอราคา</td>
			<td style="font-size:1.5em; text-align:left; padding:1.5%;border-right:1px solid #000;"><?php echo $rs_acc['name']?></td>
		</tr>
		<tr>
		<?php
		...
		?>
			...
			<td rowspan="2" colspan="2" style="font-size:1.5em; text-align:left; padding:1.5%; border-right:1px solid #000;"><?php echo $rs_address['address_no'].'&nbsp;&nbsp;'.$rs_address['road'].'&nbsp;&nbsp;'.$rs_address['sub_district'].'&nbsp;&nbsp;'.$rs_address['district'].'&nbsp;&nbsp;'.$rs_address['province'].'&nbsp;&nbsp;'.$rs_address['postal_code']?></td>
		...
		<tr>
			...
			<td colspan="2" style="font-size:1.5em; text-align:left; padding:1.5%; border-bottom:1px solid #000; border-right:1px solid #000;"><?php echo $rs_contact['email']?></td>
		...
		<tr>
			...
			<td style="font-size:1.5em; text-align:left; padding:1.5%; border-bottom:1px solid #000; border-right:1px solid #000;">ผลิตภัณฑ์ 
				<?php 
				$i_blister=0;
				...
				echo '&nbsp;&nbsp;&nbsp;';
				echo '<br>';
				echo '<div style="float:left;margin-right:2%;color:#0440DA;">';
				echo 'เลขสารบบ อย.';
						...
						while($rs_jsp_blister=mysql_fetch_array($res_jsp_blister)){
							if($detail_blister[$i_blister]==$rs_jsp_blister['id_jsp_pack_blister']){
								if(($detail_blister[$i_blister]==1) || ($detail_blister[$i_blister]==2)){echo '';}
									...
									else{if($rs_jsp_blister['description']=='Delivery'){echo 'Delivery BKK and vieinity area';}else{echo $rs_jsp_blister['description'];}}
					...
					if($rs_pack_blister['type_pack']==1){
						...
						for($i_blister==-1;$i_blister<$count_blister;$i_blister++){
							if($detail_blister[$i_blister]==1){echo '';}
								...
								elseif($detail_blister[$i_blister]==8){echo '';}
						...
						for($i_blister==-1;$i_blister<$count_blister;$i_blister++){
							if($detail_blister[$i_blister]==9){echo '';}
								...
								elseif($detail_blister[$i_blister]==16){echo '';}																	
				...
				echo 'ส่วนประกอบที่สำคัญต่อ 1 Unit';
				echo '</span>';
				echo '<br>';
				...
				$sql_npd_rela .=" order by quantities asc";
		...
		<tr>
			...
			<td colspan="2" rowspan="4" style="vertical-align: top; font-size:1.5em; text-align:left; padding:1.5%; border-top:1px solid #000; border-bottom:1px solid #000; border-right:1px solid #000;">
				...
				<?php if(in_array('1',$remark)){echo 'ลูกค้า supply '.$rs_quo['customer_supply'].'<br>';}?>
				...
				<?php if(in_array('4',$remark)){
					...
					if($rs_costing['id_product_appearance']==1){echo '150,000  เม็ด แต่ในกรณีที่เป็น Lot 1 ทางบริษัทฯอนุโลมลดให้เหลือครึ่งหนึ่ง';}
					//Capsule
					elseif($rs_costing['id_product_appearance']==2){echo '150,000 แคปซูล แต่ในกรณีที่เป็น Lot 1 ทางบริษัทฯอนุโลมลดให้เหลือครึ่งหนึ่ง';} 
					//Softgel
					elseif($rs_costing['id_product_appearance']==3){echo '150,000 แคปซูล';}
					//Instant Drink
					elseif($rs_costing['id_product_appearance']==4){echo '250,000 ซอง';}
					//Functional Drink
					elseif($rs_costing['id_product_appearance']==6){echo '150,000 ขวด' ;}
				 }
				?>
			</td>
		...
		<tr>
			<td colspan="3" rowspan="2" style="vertical-align: top; font-size:1.5em; text-align:left; padding:1.5%; border-left:1px solid #000; border-top:1px solid #000; border-bottom:1px solid #000; border-right:1px solid #000;"><b>เงื่อนไขอื่นๆ</b><br>
				...
				(2) กรุณาโอนเงินเข้าบัญชีธนาคารกรุงเทพ เลขที่บัญชี 195-472-2441<br>
				ชื่อบัญชี บริษัท ซีดีไอพี (ประเทศไทย) จำกัด สาขาถนนรัชดา-สี่แยกสาธุประดิษฐ์<br>
