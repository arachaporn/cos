	...
	<div class="row">
		...
		<table style="border: 0; width: 100%;" cellpadding="0" cellspacing="0">
			<tr>
				...
				<td class="b-bottom"><div class="large-4 columns"><h4>Call Report</h4></div></td>
			</tr>
			<?php if($rs_account['role_user'] != 3){?>
			<tr>
				<td class="b-bottom" colspan="2">
					<form name='frm_calendar' method="post" action="excel-call-report.php">
						<div class="large-4 columns">
							...
							<div style="float:left;margin:0.6% 0 0 0;"><input type="submit" name="export_excel" value="Export Excel"></div>
						</div>
					</form>
				</td>
			</tr>
					...
					<div class="large-4 columns">
						...
						<script type='text/javascript'>
							function goTo(month, year){
								window.location.href = "call-report.php?year="+ year +"&month="+ month;
							}
						</script>
						<?php
							/*echo "
							...
							*/ 
							...
							echo '<div id="main">';
							echo '<div id="nav" style="background:#70A0F5;"><button class="navLeft" style="background:#1B48D6;" onclick="goTo(\''.$prevMon.'\', \''.$prevYear.'\');">'.$prevMonText.'</button>
								<div class="title_calendar">'.$title.'</div>
								<button class="navRight" style="background:#1B48D6;" onclick="goTo(\''.$nextMon.'\', \''.$nextYear.'\');">'.$nextMonText.'</button></div>
							...
							<?php
							if(($rs_account['role_user']==1)||($rs_account['role_user']==2)){$and=' group by create_by';}else{$and=' and create_by='.$rs_account['id_account'];}							
							$j=0;
							for($i=1; $i <= $lastDay; $i++){ //วนลูป ตั้งแต่วันที่ 1 ถึงวันสุดท้ายของเดือน
								...
								$sql .=" on call_report.id_call_report=call_report_date.id_call_report";
								$sql .=" and date_visited='".$zero."' and month_visited='".$month."'";
								...
								echo "<div style='".$text_center.$back."'><a class='various' data-fancybox-type='iframe' href='ac-call-report.php?date_visited=$zero&month_visited=$month&year_visited=$year'>".$i."</a></div>";
								echo '<br>';									
								$sql="select * from call_report inner join call_report_date on call_report.id_call_report=call_report_date.id_call_report and  date_visited='".$zero."' and month_visited='".$month."' and year_visited='".$year."'";
								$sql .=$and;
								$res=mysql_query($sql) or die ('Error '.$sql);
								while($rs=mysql_fetch_array($res)){
									...
									echo '<a class="various" data-fancybox-type="iframe" style="color:#000;"href="call-report-list.php?create_by='.$rs['create_by'].'&date='.$zero.'&month='.$month.'&year='.$year.'"><div style='.$title_header.'>';
									if($rs_account['role_user']==3){echo $rs['title_call_report'];}
									if(($rs_account['role_user']==1) || ($rs_account['role_user']==2)){
