...
while($rs=mysql_fetch_array($res)){
	...
	$res_call=mysql_query($sql_call) or die ('Error '.$sql_call);
	...
	if($rs_call['type_office']==1){
		...
		$sql_in_office .=" where company_name like '".$company."' and type_office='1'";
		$res_in_office=mysql_query($sql_in_office) or die ('Error '.$sql_in_office);
	...
	if($rs_call['type_office']==2){
		$sql_in_office="select count(type_office) as sum_in_office from call_report";
		$sql_in_office .=" where company_name like '".$company."' and type_office='2'";
		$res_in_office=mysql_query($sql_in_office) or die ('Error '.$sql_in_office);
		$rs_in_office=mysql_fetch_array($res_in_office);
		$sum_in_office=0;
		...
		if(in_array('1',$status_office2)){
			...
			$sql_visited .=" where company_name like '".$company."'";
			$res_visited=mysql_query($sql_visited) or die ('Error '.$sql_visited);
		...
		if(in_array('2',$status_office2)){
			...
			$sql_booth .=" where company_name like '".$company."'";
			$res_booth=mysql_query($sql_booth) or die ('Error '.$sql_booth);
			$rs_booth=mysql_fetch_array($res_booth);
			$sum_booth=$rs_booth['sum_booth'];
		}
		if(in_array('3',$status_office2)){
			//$sum_leave=$sum_leave+1;
			$sql_leave="select count(status_leave) as sum_booth from call_report";
			$sql_leave .=" where company_name like '".$company."'";
			$res_leave=mysql_query($sql_leave) or die ('Error '.$sql_booth);
