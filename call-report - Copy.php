...
<body>
	...
	$sql .=" order by id_call_report desc LIMIT $Page_Start , $Per_Page";
	$res = mysql_query($sql) or die ('Error '.$sql);
	?>
	<div class="row">
		...
		<table style="border: 0; width: 100%;" cellpadding="0" cellspacing="0">
			<tr>
				<td class="b-bottom" style="text-align:left;">
					<div class="large-4 columns"><a href="sales-marketing.php"><?php echo '< Sales & Marketing'?></a></div>	
				</td>
				<td class="b-bottom"><div class="large-4 columns"><h4>Call Report</h4></div></td>
				<td class="b-bottom" style="text-align:right;">
					<div class="large-4 columns">
						<input type="button" value="Create" class="btn-new" onclick="window.location.href='ac-call-report.php?id_u=New'" title="New">
					...
					<div class="large-4 columns">
						<table style="border: 1 solid #eee; width: 100%;" cellpadding="0" cellspacing="0">
							<tr class="top">
								...
								<td width="15%" class="center">Month</td>
								<td width="25%"><input type="checkbox" name="ck_all" id="ck_all" value="Y" onclick="ckall(this);" style="margin:0 40% 0 0;padding:0;">Title</td>
								<?php if($rs_account['role_user'] != 3){?><td width="10%" class="center">Create by</td>
							...
							<tr>
								...
								<td>
									<?php
										$sql_date="select * from call_report_date where id_call_report='".$rs['id_call_report']."' and time_status='1'";
										$res_date=mysql_query($sql_date) or die ('Error '.$sql_date);
										$rs_date=mysql_fetch_array($res_date);
										list($ckyear,$ckmonth,$ckday) = split('[/.-]', $rs_date['visited_date']); 
										echo func_conv_status($ckmonth); 
									?>
								</td>
								<td>
									<?php
									$sql_title="select * from call_report";
									$sql_title .= $where;
									$res_title=mysql_query($sql_title) or die ('Error '.$sql_title);
									while($rs_title=mysql_fetch_array($res_title)){
									?>
									<input type="checkbox" name="ck_del[]" id="ck_del<?php echo $i?>" value="<?php echo $rs['id_call_report'];?>">
									<a href="ac-call-report.php?id_u=<?php echo $rs['id_call_report']?>"><?php echo $rs_title['title_call_report'];?></a><br>
									<?php }?>
								</td>
