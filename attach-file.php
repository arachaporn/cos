...
if($_GET["action"] == "del"){
	...
	$sql .="where id_company_file = '".$_GET["id_p"]."'";
...
<body>
	<div style="margin: 2%;">
		<form name="frm" method="post" action="dbattach-file.php" enctype="multipart/form-data">
			<input type="hidden" name="id_comp" value="<?php echo $id_com?>">
			<input type="file" name="filUpload">
			<input name="btnSubmit" type="button" value="Upload file" OnClick="JavaScript:return fncSubmit();" class="button-create">
			<?php
			$id=substr($_REQUEST['id_com'], -2);
			$sql="select * from company_file where id_company='".$id."'";
			$res=mysql_query($sql) or die ('Error '.$sql);
			...
			$sql .=" order by id_company_file desc LIMIT $Page_Start , $Per_Page";
			$res=mysql_query($sql) or die ('Error '.$sql);
			?>	
			...
			?>	
				<tr>
					...
					<td style="border-bottom:1px solid #eee;border-right:1px solid #eee;padding: 1%;"><?php echo $rs['company_file']?><span style="margin-left:2%;"><a href="JavaScript:if(confirm('Confirm Delete?')==true){window.location='<?=$_SERVER["PHP_SELF"];?>?id_p=<?php echo $rs['id_company_file'] ?>&action=del&id_file=<?php echo $id?>';}"><img src="img/delete.png" style="width:15px;" title="Delete"></a></span></td>
				</tr>
			<?php } ?>
			</table>
