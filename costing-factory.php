...
<body>
	...
	if($rs_account['role_user']==3){
		$where=" and create_by='".$rs_account['id_account']."'";
	}
	else{ $where="";}
		
	$sql="select * from costing_factory";
	$sql .=" where id_manufacturer='".$_GET['fac']."'";
	$sql.= $where;
	...
	$sql .=" order by id_costing_factory desc LIMIT $Page_Start , $Per_Page";
	$res = mysql_query($sql) or die ('Error '.$sql);
	$factory=$_GET['fac'];
	?>
	<div class="row">
			...
			<tr>
				...
				<td class="b-bottom" style="text-align:right;">
					<div class="large-4 columns">
						...
						<input type="button" value="Create" class="btn-new" onclick="window.location.href='ac-costing-table.php?id_u=New&fac=1&p=1'">
					...
					<div class="large-4 columns">
						<table style="border: 1 solid #eee; width: 100%;" cellpadding="0" cellspacing="0">
							...
							<tr>
								...
								<td class="center"><input type="checkbox" name="ck_del[]" id="ck_del<?php echo $i?>" value="<?php echo $rs['id_costing_factory'];?>"></td>
								<td><?php echo $i;?></td>
								<?php
								if($rs['id_product']==0){$product=$rs['product_name'];}
								else{
								...
								$product=$rs_product['product_name'];
								}
								?>
								<td><a href="ac-costing-table.php?id_u=<?php echo $rs['id_costing_factory']?>&fac=<?php echo $rs['id_manufacturer']?>&p=1"><?php echo $product?></a></td>
								<?php
								$sql_company="select * from company where id_company='".$rs_product['id_company']."'";
								$res_company=mysql_query($sql_company) or die ('Error '.$sql_company);
								$rs_company=mysql_fetch_array($res_company);
								?>
								<td><?php echo $rs_company['company_name']?></td>
								<td><a href="pdf-costing-thai.php?id_u=<?php echo $rs['id_costing_factory']?>&fac=<?php echo $factory?>" target=_blank><img src="img/icon-pdf-th.png" width="28px"></a>
								<a href="pdf-costing-eng.php?id_u=<?php echo $rs['id_costing_factory']?>&fac=<?php echo $factory?>" target=_blank><img src="img/icon-pdf.png" width="28px"></a></td>
								<td class="center">
								<?php 
