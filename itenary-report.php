	...
	<div class="row">
		...
		<table style="border: 0; width: 100%;" cellpadding="0" cellspacing="0">
			<tr>
				...
				<td class="b-bottom"><div class="large-4 columns"><h4>Itenary Report</h4></div></td>
						...
						<?php
							...
							*/ 
							...
							for($i=1; $i <= $lastDay; $i++){ //วนลูป ตั้งแต่วันที่ 1 ถึงวันสุดท้ายของเดือน
								$sql="select * from itenary_report where date_visited='".$i."' and month_visited='".$month."' and year_visited='".$year."'";
								$sql .=$and;
								...
								echo "<div style='".$text_center.$back."'><a class='various' data-fancybox-type='iframe' href='ac-itenary-report.php?date_visited=$i&month_visited=$month&year_visited=$year'>".$i."</a></div>";
								echo '<br>';							
								$sql="select * from itenary_report where date_visited='".$i."' and month_visited='".$month."' and year_visited='".$year."'";
								$sql .=$and;
								$res=mysql_query($sql) or die ('Error '.$sql);
								while($rs=mysql_fetch_array($res)){
									...
									echo '<a class="various" data-fancybox-type="iframe" style="color:#000;"href="ac-itenary-report.php?id_p='.$rs['id_itenary'].'"><div style='.$title_header.'>'.$rs['time_visited'].'.00&nbsp;';
