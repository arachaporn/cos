...
Changelog:
v1.7.4	12-07-08
	added timezone adjustment from Calendarix settings
	
v1.7.3	07-02-06
	added "cal" namespace for calendarix use in RSS2.0
	added sendFile function to stream file instead of requiring a file to be saved
...
// CALENDARIX MOD FOR TIMEZONE

$TZ = "+00:00";
if (isset($DTCONFIG))
{	
	$TZ = substr($DTCONFIG[2],0,3).":".substr($DTCONFIG[2],4,2);
}
define("TIME_ZONE",$TZ);
...
/**
 * A FeedCalItem is a part of a FeedCreator feed for "cal" namespace.
 *
 * @author Vincent Hor <webmaster@calendarix.com>
 * @since 1.7.2
 */
class FeedCalItem extends HtmlDescribable {
	/**
	 * Mandatory attributes of an item.
	 */
	var $ev_title, $ev_cat, $day, $month, $year, $wkday, $ev_link;
	 ...
	 */
	var $ev_location, $ev_email, $ev_url, $ev_start, $ev_end, $ev_timezone;
...
class UniversalFeedCreator extends FeedCreator {
	...
	function sendFeed($format="RSS0.91", $filename="rss.xml") {
		$this->_setFormat($format);
		$this->_feed->sendFeed($filename);
	}
...
class FeedCreator extends HtmlDescribable {
	...
	*/
	...
	/**
	 * Used by calendarix in "cal" namespace.
	 *
	 */
	var $calItems = Array();

	/**
	 * Adds an FeedCalItem to the feed.
	 *
	 * @param object FeedCalItem $item The FeedCalItem to add to the feed.
	 * @access public
	 */
    function addCalItem($item) {
		$this->calItems[] = $item;
    }
	...
	function sendFeed($filename="rss.xml") {
		$Filename = $filename;
...
class RSSCreator091 extends FeedCreator {
	...
	function createFeed() {
		...
		$feed.= "    <channel>\n";
		$feed.= "        <title>".FeedCreator::iTrunc(htmlspecialchars($this->title),300)."</title>\n";
		...
		if ($this->image!=null) {
			$feed.= "        <image>\n";
			$feed.= "            <url>".$this->image->url."</url>\n"; 
			$feed.= "            <title>".FeedCreator::iTrunc(htmlspecialchars($this->image->title),300)."</title>\n"; 
			$feed.= "            <link>".$this->image->link."</link>\n";
			if ($this->image->width!="") {
		...
		if ($this->copyright!="") {
			$feed.= "        <copyright>".FeedCreator::iTrunc(htmlspecialchars($this->copyright),300)."</copyright>\n";
		}
		if ($this->editor!="") {
			$feed.= "        <managingEditor>".FeedCreator::iTrunc(htmlspecialchars($this->editor),300)."</managingEditor>\n";
		}
		if ($this->webmaster!="") {
			$feed.= "        <webMaster>".FeedCreator::iTrunc(htmlspecialchars($this->webmaster),300)."</webMaster>\n";
		}
		if ($this->pubDate!="") {
...
class RSSCreator20 extends RSSCreator091 {
	...
	function createFeed() {
		...
		for ($i=0;$i<count($this->calItems);$i++) {
			$feed.="        <cal:calendarix xmlns:cal='".$this->calItems[$i]->calNameSpace."'>";
			$feed.="            <cal:ev_title>".FeedCreator::iTrunc(htmlspecialchars(strip_tags($this->calItems[$i]->ev_title)),300)."</cal:ev_title>\n";
			$feed.="            <cal:ev_cat>".FeedCreator::iTrunc(htmlspecialchars(strip_tags($this->calItems[$i]->ev_cat)),300)."</cal:ev_cat>\n";
			$feed.="            <cal:ev_description>".$this->calItems[$i]->getDescription()."</cal:ev_description>\n";
			$feed.="            <cal:ev_location>".FeedCreator::iTrunc(htmlspecialchars(strip_tags($this->calItems[$i]->ev_location)),300)."</cal:ev_location>\n";
			$feed.="            <cal:ev_email>".$this->calItems[$i]->ev_email."</cal:ev_email>";
			$feed.="            <cal:ev_url>".$this->calItems[$i]->ev_url."</cal:ev_url>";
			$feed.="            <cal:day>".$this->calItems[$i]->day."</cal:day>";
			$feed.="            <cal:month>".$this->calItems[$i]->month."</cal:month>";
			$feed.="            <cal:year>".$this->calItems[$i]->year."</cal:year>";
			$feed.="            <cal:wkday>".$this->calItems[$i]->wkday."</cal:wkday>";
			$feed.="            <cal:ev_link>".$this->calItems[$i]->ev_link."</cal:ev_link>";
			$feed.="            <cal:ev_start>".$this->calItems[$i]->ev_start."</cal:ev_start>";
			$feed.="            <cal:ev_end>".$this->calItems[$i]->ev_end."</cal:ev_end>";
			$feed.="            <cal:ev_timezone>".$this->calItems[$i]->ev_timezone."</cal:ev_timezone>";
			$feed.="        </cal:calendarix>";
		}
		for ($i=0;$i<count($this->items);$i++) {
			$feed.= "        <item>\n";
...
class MBOXCreator extends FeedCreator {
	...
	function createFeed() {
		for ($i=0;$i<count($this->items);$i++) {
			...
			$feed.= "Subject: ".MBOXCreator::qp_enc(FeedCreator::iTrunc($this->items[$i]->title,300))."\n";
			$feed.= "\n";
	...
	function createFeed() {
		...
		if ($this->image!=null) {
			$imageStr = "<a href='".$this->image->link."'".$targetInsert.">".
							...
							FeedCreator::iTrunc(htmlspecialchars($this->image->title),300).
		...
		if ($this->title) {
			$feedArray[] = "<div class='".$this->stylePrefix."title'><a href='".$this->link."' ".$targetInsert." class='".$this->stylePrefix."title'>".
				FeedCreator::iTrunc(htmlspecialchars($this->title),300)."</a></div>";
		}
			...
			if ($this->items[$i]->title) {
				if ($this->items[$i]->link) {
					$feedArray[] = 
						...
						"item_title'".$targetInsert.">".FeedCreator::iTrunc(htmlspecialchars(strip_tags($this->items[$i]->title)),300).
						"</a></div>";
				} else {
					$feedArray[] = 
						"<div class='".$this->stylePrefix."item_title'>".
						FeedCreator::iTrunc(htmlspecialchars(strip_tags($this->items[$i]->title)),300).
						"</div>";
				}
			}
