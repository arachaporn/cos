...
$sql_department="select * from department where id_department='".$rs_account['id_department']."'";
	...
	<div class="row">
		...
		<table style="border: 0; width: 100%;" cellpadding="0" cellspacing="0">
			<tr>
				...
				<td class="b-bottom"><div class="large-4 columns"><h4>Sales Balance</h4></div></td>
				<td class="b-bottom"><div class="large-4 columns">
					<?php if($_GET['month']){$month=$_GET['month'];$month_name=date('M', mktime(0, 0, 0, $month));}else{$month=date('m');$month_name=date('M');}
					if($_GET['year']){$year=$_GET['year'];}else{$year=date('Y');}
					?>
					<span class="button-create" style="color:#fff;"><a class='various' data-fancybox-type='iframe' href='ac-sales-target.php?month=<?=$month?>&year=<?=$year?>&st=balance' style="color:#fff;">Target of <?php echo $month_name?></a></span>
					<input type="button" name="update_data" id="update_data" value="Export PDF <?php echo $month_name?>" class="button-create" Onclick="window.open('excel-sales-balance.php?month=<?=$month?>&year=<?=$year?>')">
					<!--<input type="button" name="update_data" id="update_data" value="Export Data <?php echo $month_name?>"  class="button-create" Onclick="window.open('pdf-sales-balance1.php?month=<?=$month?>&year=<?=$year?>')">-->
					</div>
				</td>
			</tr>
			<tr>
					...
					<div class="large-4 columns">
						...
						<script type='text/javascript'>
							function goTo(month, year){
								window.location.href = "sales-balance.php?year="+ year +"&month="+ month;
							}
						</script>
						<?php
							/*echo "
							...
							*/ 
							...
							echo '<div id="nav"><button class="navLeft" onclick="goTo(\''.$prevMon.'\', \''.$prevYear.'\');">'.$prevMonText.'</button>
								<div class="title_calendar">'.$title.'</div>
							...
							?>							
							<?php
							if(($rs_account['role_user']==1)||($rs_account['role_user']==2)){$and='';}else{if($rs_account['id_account']==28){$and='';}else{$and=' and create_by='.$rs_account['id_account'];}}							
							$j=0;
							for($i=1; $i <= $lastDay; $i++){ //วนลูป ตั้งแต่วันที่ 1 ถึงวันสุดท้ายของเดือน
								$sql="select * from sm_sales_balance where date_visited='".$i."' and month_visited='".$month."' and year_visited='".$year."'";
								$sql .=$and;
								...
								echo "<div style='".$text_center.$back."'><a class='various' data-fancybox-type='iframe' href='ac-sales-balance.php?date_visited=$i&month_visited=$month&year_visited=$year'>".$i."</a></div>";
								echo '<br>';							
								$sql="select * from sm_sales_balance where date_visited='".$i."' and month_visited='".$month."' and year_visited='".$year."'";
								$sql .=$and;
								$res=mysql_query($sql) or die ('Error '.$sql);
								while($rs=mysql_fetch_array($res)){
									$j++;
									if($rs['po_reject']==1){$style_reject='<span style="color:red;">';$style_end_reject='</span">';}
									...
									echo '<a class="various" data-fancybox-type="iframe" style="color:#000;"href="ac-sales-balance.php?id_p='.$rs['id_sales_balance'].'&date_visited='.$i.'&month_visited='.$month.'&year_visited='.$year.'"><div style='.$title_header.'>';
									...
									echo $style_reject.$rs_company['company_name'].$style_end_reject;
									echo '&nbsp;&nbsp;PO.'.$rs['po_no'];
									echo '</a></div>';
								 }
